...
class UserSessions(Resource):
    def get(self):
        user_sessions = [u.to_dict() for u in UserSession.query.all()]
    ...
    def post(self):
        ...
        try:
            ...
            if user_id:
                new_user_session = UserSession(user_id=user_id, login_time=login_time, logout_time=logout_time)
...
class Timeframes(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            usersession_id = fields['usersession_id']
            if timeframe:
                new_timeframe = TimeframeModel(timeframe=timeframe, sales_amount = sales_amount, sales_units = sales_units, usersession_id = usersession_id)
...
class Categories(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if name:
                new_cat = Category(name = name, store_id = store_id, product_id = product_id)
...
class Products(Resource):
    def get(self):
        products = [p.to_dict() for p in Products.query.all()]
...
class Stores(Resource):
    def get(self):
        stores = [s.to_dict() for s in Store.query.all()]
    ...
    def post(self):
        ...
        try:
            ...
            category_id = fields['category_id']
            if name and number:
                new_store = Store(name = name, number = number, address = address, city = city, state = state, zipcode = zipcode, market = market, category_id = category_id) 
...
class Sales(Resource):
    def get(self):
        sales = [s.to_dict() for s in Sales.query.all()]
        return make_response(sales, 200)
    def post(self):
        fields = request.get_json()
        try:
            ...
            if sale_amount and sale_units:
                new_sale = Sales(sale_amount = sale_amount, sale_units = sale_units, category_id = category_id, store_id = store_id)
...
class salesById(Resource):
    def get(self, id):
        sales = Sales.query.filter_by(id=id).first()
  
        if sales == None:
            sales_dict = {"errors": ["sales not found"]}
            return make_response(sales_dict, 404)
        else:
            return make_response(sales.to_dict(), 200)
    ...
    def patch(self, id):
        ...
        if sales == None:
            ...
            try:
                for attr in fields:
                    setattr(sales, attr, fields[attr])
    ...
    def delete(self, id):
        ...
        if sales == None:
            sales_dict = {"errors": ["user not found"]}
